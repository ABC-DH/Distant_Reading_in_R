...
# first, syuzhet tokenize the sentence (with a basic regular expression)
char_v <- unlist(strsplit(my_sentence, "[^A-Za-z']+"))
char_v

# second, it calculates the value with this function:
sum(syuzhet_dict[which(syuzhet_dict$word %in% char_v), "value"])
# which checks if words in the dictionary are in the sentence
# NOT if words in the sentence are in the dictionary

# Let's find a better approach with UDpipe
library(udpipe)
library(tidyverse)
